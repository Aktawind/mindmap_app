...
from ui.theme import get_palette_for_scene, is_dark_mode_for_scene, adapt_fill, adapt_stroke
...
# haut-gauche sur la grille, TOUS les bords (gauche, droit, haut, bas) se retrouvent alignés
# automatiquement, sans avoir à choisir "au plus proche" entre deux bords à chaque micro-
# déplacement — c'est cette ancienne logique de choix qui provoquait le tremblement du nœud
# pendant le glisser (le bord choisi pouvait basculer d'une étape à l'autre).
GRID_SIZE = 20


def round_up_to_grid(value, grid_size=GRID_SIZE):
    return int(math.ceil(value / grid_size) * grid_size)


def snap_pos_to_grid(pos, rect, grid_size=GRID_SIZE):
    """Aligne le coin haut-gauche d'un nœud sur la grille virtuelle. Comme la taille de tout
    nœud est elle-même un multiple de `grid_size` (voir recalculate_size), ses quatre bords
    se retrouvent alignés sur la grille par construction, sans ambiguïté ni calcul "au plus
    proche" instable pendant un glisser."""
...
class NodeItem(QGraphicsItem):
    ...
    def recalculate_size(self):
        ...
        if self.image_path and os.path.exists(self.image_path):
            ...
            if img_w > 0 and img_h > 0:
                padding = 20
                width = round_up_to_grid(img_w + padding)
                height = round_up_to_grid(img_h + padding)
        ...
        width = round_up_to_grid(width)
        height = round_up_to_grid(height)
    ...
    def paint(self, painter, option, widget=None):
        ...
        final_border = adapt_stroke(final_border, dark)
        if dark:
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.GraphicsItemChange.ItemPositionHasChanged:
            ...
            if scene and getattr(scene, 'snap_to_grid', False):
                snapped = snap_pos_to_grid(value, self.rect, grid_size=GRID_SIZE)
...
class EdgeItem(QGraphicsPathItem):
    ...
    def paint(self, painter, option, widget=None):
        line_color = adapt_stroke(self.color, is_dark_mode_for_scene(self.scene()))
